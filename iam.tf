...
resource "aws_iam_role_policy_attachment" "internal_policies" {
  role       = aws_iam_role.task_execution_role.name
  policy_arn = aws_iam_policy.internal_policies.arn
}

data "aws_iam_policy_document" "internal_policies" {
  statement {
...
resource "aws_iam_policy" "internal_policies" {
  name   = "InternalTaskExecutionPolicies"
  path   = "/"
  policy = data.aws_iam_policy_document.internal_policies.json
}
